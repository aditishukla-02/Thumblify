...
const PreviewPanel: React.FC<Props> = ({
  ...
  return (
    <div className="relative mx-auto w-full max-w-2xl">
      <div
        className={`relative overflow-hidden rounded-xl border border-white/10 bg-black/20 ${aspectClasses[aspectRatio]}`}
      >
        {/* Loading State */}
        {isLoading && (
          ...
          <div className="group relative h-full w-full">
            ...
            <div className="absolute inset-0 flex items-end justify-center bg-black/10 opacity-0 transition-opacity group-hover:opacity-100">
              <button
                onClick={onDownload}
                type="button"
                className="mb-6 flex items-center gap-2 rounded-md px-5 py-2.5 text-xs font-medium
                bg-white/30 ring-2 ring-white/40 backdrop-blur
